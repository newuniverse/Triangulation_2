...
int main (int argc, char ** argv)
{
    ...
    label.QueueFunc(p);
    fs.writeFile(label.gCenterI, label.gCenterJ, label.r, "regular_input.txt");
    fs.writeFile(p);
    ...
    opglR.readMesh("triangulation.txt");
    
    opglR.normalizeSize(4);
    opglR.computeTriangleNormals();
    opglR.computeVertexNormals();
    opglR.computeLaplacians();
    
    opglR.qw = 1;
    opglR.qx = opglR.qy =  opglR.qz = 0;
    opglR.zoom = 1;
    opglR.shiftX = opglR.shiftY = 0;
    opglR.wire = true;
    opglR.circle = false;
    opglR.flat = false;
    cur = false;
    ...
    glutReshapeFunc(opglR.myReshape);
    glutDisplayFunc(opglR.display);
    glutMouseFunc(opglR.mouse);
    glutKeyboardFunc(opglR.keyboard);
