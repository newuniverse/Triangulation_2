...
void opengl_Rendering::allocateMem(){
...
void opengl_Rendering::readMesh(char* file_name){
...
void opengl_Rendering::normalizeSize(float length){
...
void opengl_Rendering::computeTriangleNormals(){
    for(int i=0; i<triN; i++){
        int* t = tri[i];
        float* A = ver[ t[0] ];
        float* B = ver[ t[1] ];
...
void opengl_Rendering::computeVertexNormals(){
...
void opengl_Rendering::computeLaplacians(){
    for(int i=0; i<verN; i++){
        lap[i][0] = 0;
        lap[i][1] = 0;
        lap[i][2] = 0;
...
void opengl_Rendering::laplacianSmoothing(){
    computeLaplacians();
    float dt = 0.5;
    for(int i=0; i<verN; i++){
...
void opengl_Rendering::drawWire(){
...
void opengl_Rendering::drawPoint(){
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glDisable(GL_LIGHTING); //ÉâÉCÉgÇ OFF
...
void opengl_Rendering::drawCircle(){
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glDisable(GL_LIGHTING); //ÉâÉCÉgÇ OFF
...
void opengl_Rendering::flatShading(){
...
void opengl_Rendering::smoothShading(){
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
...
void opengl_Rendering::curvature(){
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    glShadeModel(GL_SMOOTH);
...
void opengl_Rendering::display(void){
...
void opengl_Rendering::deleteMesh(){
    delete[] ver;
    delete[] tri;
    delete[] rad;
    delete[] norV;
...
void opengl_Rendering::myReshape(int w, int h){
    width = w;
    height = h;
...
//É}ÉEÉXëÄçÏ
void opengl_Rendering::mouse(int btn, int state, int x, int y){
    if(state == GLUT_DOWN){
        mouseX = x;
        mouseY = y;
    }
...
void opengl_Rendering::keyboard(unsigned char key, int x, int y){
    switch (key) {
        case 'w':
...
void opengl_Rendering::glutControl(){
    //glutInit(&argc, argv);
